...
async def processRawAndAddDateBase(schoolCode, rawData, isPublic=True):
    ...
    gitudeLatitude = rawData.get('gitudeLatitude')
    if gitudeLatitude is None:
        logger.warning(f"RunPath {runPathId} has no gitudeLatitude field, available keys: {list(rawData.keys())}")
        return
