...
class TsnRunServer:
    ...
    async def uploadRunPath(self, path, stepNumbers, sumDistance):
        ...
        if self.isPublic:
            ...
            if exerciseRecordId is None and isinstance(stopResp, dict):
                exerciseRecordId = stopResp.get('id') or stopResp.get('exerciseRecordId')
            if exerciseRecordId is not None:
                ...
                except Exception as verifyErr:
                    logger.warning(f'验证跑步记录失败：{verifyErr}')
